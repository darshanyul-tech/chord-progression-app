...
describe('App shell', () => {
  ...
  it('navigating into the Aural section shows its overview page first (no sidebar), then the topic shell on picking a topic card', () => {
    const { container } = render(<App />);
    const homeGrid = within(container.querySelector('.home-section-grid')!);
    fireEvent.click(homeGrid.getByRole('link', { name: /Aural Training/ }));
    // Section overview is a standalone page like Home — no syllabus sidebar.
    expect(screen.getByRole('heading', { name: 'Aural Training' })).toBeInTheDocument();
    expect(screen.queryByRole('navigation', { name: 'Syllabus' })).not.toBeInTheDocument();

    fireEvent.click(screen.getByRole('link', { name: /Chord Progressions/ }));
  ...
  it('navigating into the Theory section shows its own syllabus with note-reading active', () => {
    ...
    // Overview page first — no sidebar until you're inside a topic.
    expect(screen.queryByRole('navigation', { name: 'Syllabus' })).not.toBeInTheDocument();
