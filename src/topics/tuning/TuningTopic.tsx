import '../../styles/topics/tuning.css';
...
export function TuningTopic() {
  const settings = useTuningSettings();
  const setSettingsState = useTuningSettings.setState;
  const practice = useTuningPractice(settings);
  const choiceDefs = getTuningChoiceDefs();
      ...
      <section className="card">
        ...
        <div className="field" style={{ marginBottom: '0.75rem' }}>
          <div className="field-toggle-header">
            <span className="field-toggle-title" id="tuning-auto-advance-title">Auto-advance after answer</span>
            <label className="toggle-switch">
              <input
                type="checkbox"
                aria-labelledby="tuning-auto-advance-title"
        ...
        <p className="tuning-prompt">{practice.promptText}</p>
        ...
        <ChoiceGrid
          ...
          groupClassName="tuning-choices"
          choiceClassName="tuning-choice"
          ariaLabel="Tuning answers"
        />

        <SessionScoreLine className="tuning-session-score" correct={practice.score.correct} total={practice.score.total} />
